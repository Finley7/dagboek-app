    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="p-6">
                    <h1 class="pb-3">{{ __('Create diary article for') }}: {{ $diary->title }}</h1>
                    @if (\Session::has('success'))
                        <div
                            class="p-3 bg-green-500 rounded tracking-tight text-white">{{ \Session::get('success') }}</div>
                    @endif
                    <form method="post" action="{{ route('diary.article.store', ['diaryId' => $diary->id]) }}">
                        @csrf
                        <div>
                        ...
                        <button class="btn bg-green-500 mt-3">{{ __('Create diary article') }}</button>
                    </form>
                </div>
            </div>
        </div>
